def shuffle(s):
    ...
    half = len(s) // 2 
    result = list(sum(zip(s[:half],s[half:]),()))
    return result
...
def balanced(m):
    ...
    if(is_mobile(m)):
        if(total_mass(end(left(m)))*length(left(m)) != total_mass(end(right(m)))*length(right(m))):
            ...
            return all(map(balanced,(end(left(m)),end(right(m)))))
    return all(map(balanced,(end(left(m)),end(right(m)))))
...
def berry_finder(t):
    ...
    if(label(t)== 'berry'):
        return True
    all_branches = tuple(b for b in branches(t))
    return any(map(berry_finder , all_branches))
