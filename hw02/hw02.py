...
def count_dollars(total):
    ...
    def inner(total,k):
        ...
        if(total < 0):
            return 0
        return inner(total - k,k) + inner(total, next_smaller_dollar(k))
    return inner(total,100)
...
def shuffle(s):
    ...
    n = len(s) // 2
    return [elem for i in range(n) for elem in (s[i], s[i+n])]
...
def merge(s, t):
    ...
    len_t = len(t)
    def inner(len1,len2):
        if(len1 >= len_s and len2 < len_t):
            return t[len2:]
        if(len2 >= len_t and len1 < len_s):
            return s[len1:]
        if(s[len1] <= t[len2]):
            return [s[len1]]+inner(len1+1,len2)
        if(s[len1] > t[len2]):
            return [t[len2]]+inner(len1,len2+1)
